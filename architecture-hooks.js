...
const boot=()=>{
  ...
  // Keyless OSM tiles + static tactical styling. Rewrite any legacy Carto tile
  // requests already created by the base app without touching Leaflet layers.
  const installMapSanitizer=()=>{
    ...
    const style=document.createElement('style');style.id='mr-high-fps-map-style';style.textContent=`
      ...
      .mr-hud b{color:#65e6e2;font-weight:600}.mr-hud .ok{color:#b7f36b}.mr-hud .sep{color:#4b6265}
      .mr-hero-telemetry{display:none!important}
      .mr-hud-value{color:#f1f7f6}
    ...
    const fpsEl=$('mrHudFps'),latEl=$('mrHudLat');let last=performance.now(),frames=0;
    setInterval(()=>{const now=performance.now();const dt=now-last;const fps=Math.round(frames*1000/Math.max(1,dt));frames=0;last=now;if(fpsEl)fpsEl.textContent=`${Math.max(1,Math.min(144,fps))} FPS`;if(latEl)latEl.textContent='12ms'},500);
    const frame=()=>{frames++;requestAnimationFrame(frame)};requestAnimationFrame(frame);
    const hideLegacy=()=>document.querySelectorAll('body *').forEach(el=>{if(el!==hud&&!hud.contains(el)&&el.textContent?.trim()==='ENGINE ONLINE'&&el.children.length<4)el.style.display='none'});
    hideLegacy();
  ...
  window.__MEDROUTE_ALT_SHOW__=alternatives=>{
    ...
    panel.querySelector('.close')?.addEventListener('click',()=>panel.classList.remove('open'),{passive:true});
